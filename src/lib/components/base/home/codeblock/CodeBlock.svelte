<script lang="ts">
	...
	let { code = '', fileName = '', lang = 'svelte', class: _class = '' }: Props = $props();
</script>

<div class="bg-background border-border group relative h-full rounded-xl border p-4">
	...
	{#if fileName.length > 0}
		<div class="absolute top-4 left-4 flex gap-1">
			...
			{/if}
			<span
				class="text-sm transition-all duration-200 group-hover:text-neutral-100 dark:text-neutral-400"
				>{fileName}</span
			>
		</div>
	{/if}
	<div class={cn('relative -mt-2 overflow-x-auto', _class)}>
		<Code {code} {lang} />
	</div>
</div>
